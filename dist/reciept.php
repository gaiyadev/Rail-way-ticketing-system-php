...
"><h3>PRINT RECIEPT</h3></div>
              <div class="panel-body">
                <form class="form-horizontal" role="form" action="reciept.php" method="POST">
                    <div class="form-group">
                    <label class="control-label col-sm-4" for="ticket_number">Ticket no:</label>
                    <div class="col-sm-6">
                    <input type="text" class="form-control" id="fname" name="number" placeholder="Enter Ticket number"  required="required">
                    </div>
                    </div>
       ...
       <!--  -->
                    ...
                    if(isset($_POST['submit'])){
                      ...
                      if (!mysqli_num_rows($check) >0) {
                        ...
                        if ($ticno === $row['id'] && $row['type'] ==="regular"){
                          echo "<script>
                      window.open('regular.php?=success', '_self')
                      </script>";
                           //header("Location: regular.php");
                         }elseif($ticno === $row['id'] && $row['type'] ==="firstclass"){
                          echo "<script>
                      window.open('firstclass.php?=success', '_self')
                      </script>";
